...
- **定位改为宏观信号（Macro Signal）**：项目此前是 AI 一线信号追踪器（`Benboerba620/ai-signal` 上游），7/10 已把信源换成宏观/中国经济/投资方向，本次完成剩余改造——`README.md`/`SKILL.md`/全部 `prompts/*.md` 重写为宏观框架，不再按"AI 相关性"过滤内容。技术上仍保留 `ai-signal` 目录名，兼容已有的定时任务与本地配置。
- **触发词改为 `/资讯推送`**：手动查看日报的触发词从 `/ai-signal` 改为 `/资讯推送`，与新的宏观定位对齐。技术名称（`name: ai-signal`）、安装目录、GitHub 仓库地址、`~/.ai-signal/` 本地配置路径均保持不变，仅用户输入的触发短语变化。
